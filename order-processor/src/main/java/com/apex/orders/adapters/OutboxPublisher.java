...
public final class OutboxPublisher {
    ...
    public OutboxPublisher(MongoStore store, Sender sender) {
        this(store, sender, 3, 5000);
    }

    public OutboxPublisher(MongoStore store, Sender sender, int maxPermanentFailures, long retryDelayMs) {
        if (maxPermanentFailures < 1 || retryDelayMs < 0)
            throw new IllegalArgumentException("Invalid outbox retry settings");
        this.store = store;
        this.sender = sender;
        this.maxPermanentFailures = maxPermanentFailures;
        this.retryDelayMs = retryDelayMs;
    }

    public static Sender kafka(KafkaTemplate<String, String> kafka) {
        return (topic, key, payload) -> kafka.send(topic, key, payload).get(10, TimeUnit.SECONDS);
    }
